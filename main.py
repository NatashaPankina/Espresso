...
class Add(QMainWindow):
    ...
    def run(self):
        ...
        if len(d) == 0 or len(a) == 0 or len(e) == 0 or not (e.isdigit()) or not (f.isdigit()) or len(f) == 0:
            ...
            cur = con.cursor()
            cur.execute(
                """INSERT INTO table_coffee(sort, roast, grains, taste, price, size) VALUES (?, ?, ?, ?, ?, ?)""",
                (a, b, c, d, e, f))
            con.commit()
            con.close()
...
class Change(QMainWindow):
    ...
    def initUI(self):
        ...
        if len(rows) != 0:
            con = sqlite3.connect('coffee.sqlite')
            cur = con.cursor()
            res = cur.execute("""SELECT sort, roast, grains, taste, price, size FROM table_coffee WHERE ID = ?""",
                              (ids[0],)).fetchall()
